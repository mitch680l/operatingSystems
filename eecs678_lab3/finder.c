...
int main(int argc, char *argv[])
{
  ...
  //fprintf(stderr, "Debug message: pipe created!\n");
  ...
  if (pid_2 == 0) {
    ...
    dup2(p1[0], STDIN_FILENO); //pipe 1 read end to standard input
   
    dup2(p2[1], STDOUT_FILENO); //standard output to pipe 2 write end
  ...
  if (pid_3 == 0) {
    ...
    dup2(p2[0], STDIN_FILENO);//p2 read to stdin
    
    dup2(p3[1],STDOUT_FILENO);//stdout to p3 write
  ...
  pid_4 = fork();
  if (pid_4 == 0) {
    //fprintf(stderr, "Child4\n");
    //fprintf(stderr, "Limit for head: %s\n", argv[3]);  // Debug argv[3]
    ...
    execl(HEAD_EXEC, HEAD_EXEC, "--lines", argv[3],(char *)NULL);
    exit(0);
  }
  close(p1[0]);
  close(p1[1]);
